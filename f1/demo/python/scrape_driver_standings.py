...
try:
    ...
    for results_categorie_item in results_categorie_items:
        ...
        if results_categorie_item_anchor.text == "DRIVERS":
            print('Hover over DRIVERS link...')
            drivers_hover = ActionChains(driver).move_to_element(results_categorie_item_anchor)
            drivers_hover.perform()
            driver_results_href = results_categorie_item_anchor.get_attribute('href')
    ...
    time.sleep(5)
    # f = open("driver_results_data.json", "w")
    # f.write(json.dumps(drivers_results_years_data))
    # f.close()

    print(json.dumps(drivers_results_years_data))
